...
def test_parses_external_parameter_binding_and_mapping_references(embrace_ssd_fixture):
    ...
    assert document.system is not None
    assert len(document.system.get_parameter_bindings()) == 1
    binding = document.system.get_parameter_bindings()[0]
...
def test_round_trip_preserves_inline_and_external_parameter_bindings(mixed_ssd_fixture):
    ...
    assert document.system is not None
    assert len(document.system.get_parameter_bindings()) == 2

    inline_binding = document.system.get_parameter_bindings()[0]
    ...
    external_binding = document.system.get_parameter_bindings()[1]
    ...
    assert len(reparsed.system.get_parameter_bindings()) == 2
    assert reparsed.system.get_parameter_bindings()[0].parameter_set is not None
    assert reparsed.system.get_parameter_bindings()[0].parameter_set.parameters[0].attributes["value"] == "1.5"
    assert reparsed.system.get_parameter_bindings()[1].source == "external_values.ssv"
...
def test_round_trip_preserves_inline_parameter_mapping_document():
    ...
    assert reparsed.system is not None
    assert reparsed.system.get_parameter_bindings()[0].parameter_mapping is not None
    assert reparsed.system.get_parameter_bindings()[0].parameter_mapping.source is None
    assert reparsed.system.get_parameter_bindings()[0].parameter_mapping.mapping is not None
    assert reparsed.system.get_parameter_bindings()[0].parameter_mapping.mapping.mappings[0].target == "Plant.gain"
    assert reparsed.system.get_parameter_bindings()[0].parameter_mapping.mapping.mappings[0].suppress_unit_conversion is True
