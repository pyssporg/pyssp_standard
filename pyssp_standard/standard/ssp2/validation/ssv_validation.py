...
DEFAULT_SSP2_SSV_SCHEMA_PATH = resolve_schema_path("SSP2", "SystemStructureParameterValues.xsd")


class Ssp2SsvSchemaValidator(XmlSchemaValidator):
    def __init__(self, schema_path: Path | None = None):
        super().__init__(
            schema_path or DEFAULT_SSP2_SSV_SCHEMA_PATH,
            error_prefix="SSV XML failed XSD validation",
        )


class Ssp2SsvSemanticValidator:
    def validate(self, model: Ssp2ParameterSet) -> None:
...
class Ssp2SsvValidator:
    def __init__(
        self,
        *,
        schema_validator: Ssp2SsvSchemaValidator | None = None,
        semantic_validator: Ssp2SsvSemanticValidator | None = None,
    ):
        self.schema_validator = schema_validator or Ssp2SsvSchemaValidator()
        self.semantic_validator = semantic_validator or Ssp2SsvSemanticValidator()

    def validate(self, model: Ssp2ParameterSet, xml_text: str) -> None:
